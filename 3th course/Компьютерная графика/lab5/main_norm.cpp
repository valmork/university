...
const char *sceneTitles[] = {
    "Сцена 1: Ковёр 1x1 Ручное",
    "Сцена 2: Ковёр 2x2 Ручное",
    "Сцена 3: Ковёр 4x4 Ручное",
    "Сцена 4: Ковёр Поворот 1 Ручное",
    "Сцена 5: Ковёр Поворот 2 Ручное",
    "Сцена 6: Ковёр 1x1 Авто",
    "Сцена 7: Ковёр 2x2 Авто",
    "Сцена 8: Ковёр 4x4 Авто",
    "Сцена 9: Ковёр Поворот 1 Авто",
    "Сцена 10: Ковёр Поворот 2 Авто",
    "Сцена 11: Кубик",
    "Сцена 12: Кот"};
...
void loadTextures(TextureInfo textures[], int numTextures)
{
    ILuint imageIDs[numTextures];
    GLuint tempTextureIDs[numTextures];

    ilGenImages(numTextures, imageIDs);
    glGenTextures(numTextures, tempTextureIDs);

    for (int i = 0; i < numTextures; i++)
    {
        ilBindImage(imageIDs[i]);

        if (!ilLoadImage(textures[i].filename))
            continue;

        ilConvertImage(IL_RGBA, IL_UNSIGNED_BYTE);

        glBindTexture(GL_TEXTURE_2D, tempTextureIDs[i]);

        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ilGetInteger(IL_IMAGE_WIDTH), ilGetInteger(IL_IMAGE_HEIGHT), 0, GL_RGBA, GL_UNSIGNED_BYTE, ilGetData());

        *(textures[i].textureID) = tempTextureIDs[i];
    }

    ilDeleteImages(numTextures, imageIDs);
}

void loadMipmaps(const char *path, const char *ext, GLuint *textureID)
{
    ...
    while (true)
    {
        std::string filename = std::string(path) + "/level" + std::to_string(level) + "." + ext;
...
void loadAllTextures()
{
    TextureInfo textures[] = {
        {"bmp/1.bmp", &diceSide1Texture},
        {"bmp/2.bmp", &diceSide2Texture},
        {"bmp/3.bmp", &diceSide3Texture},
        {"bmp/4.bmp", &diceSide4Texture},
        {"bmp/5.bmp", &diceSide5Texture},
        {"bmp/6.bmp", &diceSide6Texture},
        {"bmp/128.bmp", &carpetTexture}};

    const int numTextures = sizeof(textures) / sizeof(textures[0]);
    loadTextures(textures, numTextures);

    loadMipmaps("bmp/mipmaps", "bmp", &catTexture);
}
