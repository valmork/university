data class BackwardRule(
    val id: String,
    val conditions: List<String>,
    val conclusion: String
)

data class BackwardInferenceStep(
    val stepNumber: Int,
...
class ForwardChainingEngine(
    private val rules: List<BackwardRule>
) {
    fun run(initialFacts: Set<String>): Pair<Set<String>, List<BackwardInferenceStep>> {
        val workingMemory = initialFacts.toMutableSet()
        val steps = mutableListOf<BackwardInferenceStep>()
        var stepNumber = 1
        var changed = true
...
fun printResultTable(
    ...
    steps: List<BackwardInferenceStep>,
...
fun main() {
    ...
    val rules1 = listOf(
        BackwardRule("R1",  listOf("имеет_шерсть"),"млекопитающее"),
        BackwardRule("R2",  listOf("кормит_молоком"),"млекопитающее"),
        BackwardRule("R3",  listOf("имеет_перья"),"птица"),
        BackwardRule("R4",  listOf("умеет_летать", "откладывает_яйца"),"птица"),
        BackwardRule("R5",  listOf("млекопитающее", "ест_мясо"),"плотоядное"),
        BackwardRule("R6",  listOf("млекопитающее", "имеет_когти", "зоркие_глаза"),"плотоядное"),
        BackwardRule("R7",  listOf("млекопитающее", "копыта"),"парнокопытное"),
        BackwardRule("R8",  listOf("плотоядное", "желтый_цвет", "пятна"),"гепард"),
        BackwardRule("R9",  listOf("плотоядное", "желтый_цвет", "полосы"),"тигр"),
        BackwardRule("R10", listOf("парнокопытное", "длинная_шея"),"жираф"),
        BackwardRule("R11", listOf("парнокопытное", "полосы"),"зебра"),
    )
