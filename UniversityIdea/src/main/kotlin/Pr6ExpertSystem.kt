...
data class Recommendation(
    ...
    val ram: Component,
    val gpu: Component,
    val storage: Component,
    val motherboard: Component,
    val psu: Component,
    val cooler: Component,
...
object RuleEngine {
    fun infer(facts: UserFacts): Recommendation {
        ...
        // Правила по процессору
        val cpu = when {
            highLoad && heavyBudget -> {
                exp += "Правило: Высокая нагрузка + бюджет>100к -> AMD Ryzen 9 7950X"
                KnowledgeBase.cpus["extreme"]!! }
            highLoad && facts.budget in 60_000..100_000 -> {
                exp += "Правило: Высокая нагрузка, средний бюджет -> Intel Core i5-13600K"
                KnowledgeBase.cpus["mid_gaming"]!! }
            facts.needProgramming && facts.needMultitasking -> {
                exp += "Правило: Программирование + многозадачность -> AMD Ryzen 7 5800X"
                KnowledgeBase.cpus["mid_workstation"]!! }
            facts.budget < 40_000 -> {
                exp += "Правило: Бюджетный ПК -> Intel Core i3-12100"
                KnowledgeBase.cpus["budget_basic"]!! }
            else -> {
                exp += "Правило: Универсальный ПК -> AMD Ryzen 5 5600"
                KnowledgeBase.cpus["budget_mid"]!! }
        }

        // Правила по видеокарте
        val gpu = when {
            facts.need3D && heavyBudget -> {
                exp += "Правило: Профессиональный 3D + бюджет>100к -> NVIDIA RTX A4000"
                KnowledgeBase.gpus["pro"]!! }
            facts.needGaming && heavyBudget -> {
                exp += "Правило: Топовые игры + бюджет>100к -> RTX 4090"
                KnowledgeBase.gpus["extreme"]!! }
            (facts.needGaming || facts.needVideo) && facts.budget > 60_000 -> {
                exp += "Правило: Игры/видео, бюджет>60к -> RTX 4070 Ti"
                KnowledgeBase.gpus["high"]!! }
            !facts.needGaming && !facts.need3D && !facts.needVideo -> {
                exp += "Правило: Нет графических задач -> встроенная графика"
                KnowledgeBase.gpus["integrated"]!! }
            else -> {
                exp += "Правило: Начальный уровень -> RTX 3050"
                KnowledgeBase.gpus["budget"]!! }
        }

        // Правила по оперативной памяти
        val ram = when {
            (facts.needVideo || facts.need3D) && heavyBudget -> {
                exp += "Правило: Видео/3D + бюджет>100к -> 64 GB DDR5"
                KnowledgeBase.rams["64gb"]!! }
            facts.needGaming || facts.needProgramming || facts.needMultitasking -> {
                exp += "Правило: Игры / программирование / многозадачность -> 32 GB DDR5"
                KnowledgeBase.rams["32gb"]!! }
            facts.budget > 35_000 -> {
                exp += "Правило: Стандартный объем -> 16 GB DDR4"
                KnowledgeBase.rams["16gb"]!! }
            else -> {
                exp += "Правило: Минимальный бюджет -> 8 GB DDR4"
                KnowledgeBase.rams["8gb"]!! }
        }

        // Правила по накопителю данных
        val storage = when {
            (facts.needVideo || facts.need3D) && facts.needStorage -> {
                exp += "Правило: Видео/3D + много данных -> SSD 1 TB + HDD 4 TB"
                KnowledgeBase.storages["ssd_hdd"]!! }
            facts.needGaming || facts.needStorage -> {
                exp += "Правило: Игры / много данных -> SSD NVMe 2 TB"
                KnowledgeBase.storages["ssd_2tb"]!! }
            facts.budget > 40_000 -> {
                exp += "Правило: Стандартный объём -> SSD NVMe 1 TB"
                KnowledgeBase.storages["ssd_1tb"]!! }
            else -> {
                exp += "Правило: Бюджетный вариант -> SSD NVMe 512 GB"
                KnowledgeBase.storages["ssd_512"]!! }
        }

        // Правила по материнской плате
        val mb = when {
            cpu.name.contains("7950") || cpu.name.contains("7900") -> {
                exp += "Правило: Мат плата подобрана под высокобюджетный AMD CPU"
                KnowledgeBase.motherboards["high_amd"]!!
            }
            cpu.name.contains("i7")   || cpu.name.contains("i5")   -> {
                exp += "Правило: Мат плата подобрана под среднебюджетный Intel CPU"
                KnowledgeBase.motherboards["mid_intel"]!!
            }
            cpu.name.contains("Ryzen 7") -> {
                exp += "Правило: Мат плата подобрана под среднебюджетный AMD CPU"
                KnowledgeBase.motherboards["mid_amd"]!!
            }
            cpu.name.contains("i3") -> {
                exp += "Правило: Мат плата подобрана под низкобюджетный Intel CPU"
                KnowledgeBase.motherboards["budget_intel"]!!
            }
            //Правило: Мат плата подобрана под низкобюджетный AMD CPU"
            else -> {KnowledgeBase.motherboards["budget_amd"]!!}
        }

        // Правила по блоку питания
        val psu = when {
        ...
        exp += "Правило: БП подобран по TDP видеокарты -> ${psu.name}"

        // Правила по охлаждению
        val cooler = when {
            cpu.name.contains("7950") || cpu.name.contains("7900") -> {
                exp += "Правило: Топовый CPU -> жидкостное охлаждение NZXT Kraken"
                KnowledgeBase.coolers["liquid"]!! }
            cpu.name.contains("i7") -> {
                exp += "Правило: Тихая работа / мощный CPU -> Noctua NH-U12S"
                KnowledgeBase.coolers["mid"]!! }
            cpu.name.contains("i3") -> {
                exp += "Правило: Бюджетный CPU -> боксовый кулер"
                KnowledgeBase.coolers["stock"]!! }
            else -> {
                exp += "Правило: Стандартный CPU -> DeepCool AK400"
